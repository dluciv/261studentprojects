...
public class Parser {
    ...
    private Expression prime() throws ParserException {
        ...
        if (lexer.getCurrLexeme().getTypeLexem() == Lexeme.Number) {
            ...
            if (lexer.getCurrLexeme().getStringIdLexem().equals(recfun)) {
                expr1 = new PrimeId(lexer.getCurrLexeme().getStringIdLexem());
                lexer.next();
                expr2 = getExpression();
                left = new Application(expr1, expr2);
                lexer.next();
                return left;
            } else {
                left = new PrimeId(lexer.getCurrLexeme().getStringIdLexem());
                lexer.next();
                return left;
            }
            ...
            id = new PrimeId(lexer.getCurrLexeme().getStringIdLexem());
            lexer.next();
            ...
            if (lexer.getCurrLexeme().getTypeLexem() == Lexeme.Rec) {
                lexer.next();
                idRec = new RecId(lexer.getCurrLexeme().getStringIdLexem());
