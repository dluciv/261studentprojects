...
public class Parser {
	...
	private Expression parseConjunctor() {
		...
		while (curToken.getType() == TokenType.EQUALITY
				|| curToken.getType() == TokenType.INEQUALITY) {
			Token sign = curToken;
	...
	private Expression parseMatching() {
		...
		if (curToken.getType() == TokenType.GREATER
				|| curToken.getType() == TokenType.LESS
				|| curToken.getType() == TokenType.GE
				|| curToken.getType() == TokenType.LE) {
			Token sign = curToken;
	...
	private Expression parseCompared() {
		...
		while (curToken.getType() == TokenType.PLUS
				|| curToken.getType() == TokenType.MINUS) {
			Token sign = curToken;
			...
			if (sign.getType() == TokenType.PLUS) {
				...
				result = new ArSubtraction(result, toFind, curToken
						.getPosition());
			}
		}
		return result;
	...
	private Expression parseTerm() {
		...
		while (curToken.getType() == TokenType.MULTIPLICATION
				|| curToken.getType() == TokenType.DIVISION) {
			Token sign = curToken;

			Expression toFind = parseFactor();
			if (sign.getType() == TokenType.MULTIPLICATION) {
				result = new ArMultiplication(result, toFind, curToken
						.getPosition());
			} else {
	...
	private Expression parsePrime() { // System.out.println(curToken.getType());
		...
		if (curToken.getType() == TokenType.NUMBER) {
			...
			if (curToken.getType() != TokenType.LEFT_BRACKET) {
				...
				ExPrint exPrint = new ExPrint(parseExpression(), curToken
						.getPosition());
				if (curToken.getType() != TokenType.RIGHT_BRACKET) {
