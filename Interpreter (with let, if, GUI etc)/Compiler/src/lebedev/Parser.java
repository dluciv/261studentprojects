...
public class Parser {
    ...
    public void parseProgramm() {
        ...
        if (errorCounter == 0) {
            ...
            Tool.fixError("there are lexical errors", curToken.getPosition());
            output = null;
        }
    }
    ...
    private Expression parsePrime() {
        ...
        if (curToken.getType() == TokenType.NUMBER) {
            ...
            if (curToken.getType() != TokenType.RIGHT_BRACKET) {
                Tool.fixError("strange symbol, ')' expected", curPosition);
                result = null;
            } else {
                nextToken();
                result = toFind;
            ...
            if (curToken.getType() != TokenType.ID) {
                ...
                Tool.fixError("missed identifier", curPosition);
                result = null;
            } else {
                ...
                if (curToken.getType() != TokenType.EQUALS_SIGN) {
                    ...
                    if (letExpression == null) {
                        ...
                        if (curToken.getType() != TokenType.IN) {
                            curPosition = curToken.getPosition();
                            Tool.fixError("missed IN expression", curPosition);
                            result = null;
                        } else {
                            curPosition.setEndAbs(curToken.getPosition().getEndAbs());
            ...
            if (ifExpr == null) {
                curPosition = curToken.getPosition();
                Tool.fixError("missed first condition", curPosition);
                result = null;
            } else {
                if (curToken.getType() != TokenType.THEN) {
                    ...
                    if (thenExpr == null) {
                        curPosition = curToken.getPosition();
                        Tool.fixError("missed first expression", curPosition);
                        result = null;
                    } else {
                        if (curToken.getType() != TokenType.ELSE) {
                            ...
                            if (elseExpr == null) {
                                curPosition = curToken.getPosition();
                                Tool.fixError("missed second expression", curPosition);
                                result = null;
                            } else {
                                curPosition.setEndAbs(curToken.getPosition().getEndAbs());
            ...
            if (curToken.getType() != TokenType.ID) {
                curPosition = curToken.getPosition();
                Tool.fixError("invalid variable", curPosition);
                result = null;
            } else {
                int id = curToken.getAttribute();
                nextToken();
            ...
            Tool.fixError("no expression after semicolon ", curToken.getPosition());
            result = null;
