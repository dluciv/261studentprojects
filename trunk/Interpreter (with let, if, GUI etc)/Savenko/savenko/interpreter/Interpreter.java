 ...
 * должен контактировать с классом потока вывода - пока не реализованно
 * Savenko Maria(c)
 */
package savenko.interpreter;

import savenko.ast.And;
import savenko.ast.Application;
...
public class Interpreter {
     ...
     private Value interpret(Binding binding) throws InterpreterException {
          ...
          if (environment.ifIdentifierExist(binding.getIdentifier())) {
               ...
               if (binding.getValue() != null) {
                    environment.addToEnvironment(binding.getIdentifier(), 
     ...
     private Value interpret(Application id) throws InterpreterException {
    	...
    	if (environment.ifIdentifierExist(fun.getIdentifier())) {
            ...
            if (id.getExpression() != null) {
                 ...
                 val = interpret(fun.getExpression());
                 environment.removeIdentifier(fun.getIdentifier());
            } else {
                 val = interpret(fun.getExpression());
            }
       }
